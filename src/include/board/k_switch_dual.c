...
void GPIO_ICACHE_FLASH supla_esp_board_input_state_change(void *_input_cfg) {
	...
	if ( input_cfg->last_state == 0 ) {
		if ( Licznik2 == 0 && currentDeviceState == STATE_CONNECTED && supla_esp_state.Relay[5] == 1 ) {
			Licznik2 = 1;
...
void supla_esp_board_gpio_init(void) {
	...
	supla_input_cfg[0].channel = chnl;
	...
	supla_input_cfg[1].channel = chnl+1;
...
char html_template_header[] =
  ...
  static char html_template[] =
      ...
      "update</label></i></div><button type=\"submit\">SAVE &amp; RESTART</button><input "
...
void ICACHE_FLASH_ATTR supla_esp_board_on_connect(void) {
	...
	supla_log(LOG_DEBUG, "supla_esp_state RELAY 2 = %i", supla_esp_state.Relay[2]);
